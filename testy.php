...
if(isset($_GET['code'])){
	...
	$delSql = "DELETE FROM results WHERE testCode = '$delCode'";
	mysqli_query($conn, $delSql);
    ...
    <div id="code-container">
        <div id="center-container">
            <table class="testTable">
                <tr><th>Nazwa testu</th><th>Kod</th><th>Liczba pytań</th></tr>
        <?php
        $uid = $_SESSION['uid'];
        ...
        for($i=0;$i<$row_cnt;$i++){
            ...
            $name = $row['name'];
            $code = $row['code'];
            $numQuesSql = "SELECT COUNT(*) as count FROM questions WHERE testCode = '$code'";
            $numQuesRes = mysqli_query($conn,$numQuesSql);
            $numQuesFetch = $numQuesRes->fetch_assoc();
            $numQuesRes = $numQuesFetch['count'];
            echo "<tr><td>$name</td><td>$code</td><td>$numQuesRes</td><td><a href='edytuj.php?code=$code'>Edytuj</a></td><td><a href='?code=$code'>Usuń</a></td></tr>";
        }
